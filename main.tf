resource "azurerm_windows_function_app" "windows_function_app" {
  ...
  app_settings = merge(each.value.app_settings, {
    ...
    WEBSITE_CONTENTAZUREFILECONNECTIONSTRING = format("DefaultEndpointsProtocol=https;AccountName=%s;AccountKey=%s;EndpointSuffix=core.windows.net", format("st%s", substr(each.value.name, 2, -1)), each.value.storage_account_access_key)
  })
